(ns medicaldevice.operation
  "OperationActor -- one medical device manufacturing operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor (Medical Device
  Advisor) is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Manufacturing Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one manufacturing operation (intake -> advise -> govern
  ...
  decision to a human operator (the quality engineer or device-release reviewer).
  The approver resumes with `{:approval {:status :approved}}` (or :rejected).
  High-stakes operations ALWAYS reach this node when the governor is clean --
  see `medicaldevice.phase`."
...
(defn build
  ...
  `medicaldevice.store/Store`).
  opts:
    :advisor      -- a `medicaldevice.advisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [device-store & [{:keys [advisor-impl checkpointer]
                    :or   {advisor-impl   (advisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Medical Device Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor-impl device-store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Manufacturing Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
        (fn [{:keys [request context proposal]}]
          {:verdict (governor/check request context proposal device-store)}))
      ...
      (g/add-node :commit
        (fn [{:keys [request context proposal record]}]
          (store/commit-record! device-store record)
          (let [f (commit-fact request context proposal)]
            (store/append-ledger! device-store f)
            {:audit [f]})))
      ...
      (g/add-node :hold
        (fn [{:keys [audit]}]
          (when-let [hf (last (filter #(#{:governor-hold :approval-rejected} (:t %)) audit))]
            (store/append-ledger! device-store (assoc hf :disposition :hold)))
          {}))
